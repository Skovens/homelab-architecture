...
See [ADR-0004](../../decisions/0004-podman-secrets.md) for the full decision context, and the [Security Model](../../principles/security-model.md) for the per-service isolation rationale.

!!! note
    `no_log: true` prevents Ansible from logging secret values. Without it, the generated secret would appear in playbook output.
