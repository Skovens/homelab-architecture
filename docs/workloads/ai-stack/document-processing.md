...
Docling is self-hosted, CPU-only, and handles complex PDFs well. The `UVICORN_WORKERS=1` setting is a gotcha — multiple workers cause "Task Not Found" errors with async conversion requests. This is a docling-serve limitation, not a configuration error.
